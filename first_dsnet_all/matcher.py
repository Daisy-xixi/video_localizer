...
class HungarianMatcher(nn.Module):
    ...
    """
    def __init__(self,  cost_class: float = 1, cost_span: float = 1, cost_giou: float = 1, cost_cls: float = 1, cost_loc: float = 1,
        ...
        assert cost_class != 0 or cost_span != 0 or cost_giou != 0 or cost_cls != 0 or cost_loc != 0, "all costs cant be 0"
    ...
    def forward(self, outputs, targets):
        ...
        targets_cls_label = targets["cls_label"]
        targets_loc_label = targets["loc_label"]
        ...
        # compute dsnet cost
        # out_pred_cls = outputs['pred_cls']  # [batch_size * num_queries]
        # out_pred_loc = outputs['pred_loc'].flatten(0, 1)
        
        # tgt_loc_label = targets_loc_label.flatten(0, 1)
        # tgt_cls_label = targets_cls_label.flatten(0, 1)
        ...
        if self.span_loss_type == "l1":
            ...
            # cost_pred_loc = torch.cdist(out_pred_loc, tgt_loc_label, p=1) 
            # cost_pred_cls = calc_cls_loss(out_pred_cls, tgt_cls_label)
        ...
        indices_list = []
        # for i, ch in enumerate(C.split(sizes, -1)):
        #     temp = linear_sum_assignment(ch[i])
        #     indices_list.append(temp)
...
def build_matcher(args):
    ...
    return HungarianMatcher(
        ...
        cost_cls=args.set_cost_cls, cost_loc=args.set_cost_loc,
