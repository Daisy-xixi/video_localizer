...
def compute_mr_ap(submission, ground_truth, iou_thds=np.linspace(0.5, 0.95, 10),
    ...
    for dd in submission:
        pred_windows = dd["pred_relevant_windows"][:max_pred_windows] \
            if max_pred_windows is not None else dd["pred_relevant_windows"]
        # pred_windows = dd["pred_relevant_windows_score_all"][:max_pred_windows] \
        #     if max_pred_windows is not None else dd["pred_relevant_windows_score_all"]
        qid = dd["qid"]
        for ww in pred_windows:
            pred_qid2data[qid].append({
                "video-id": dd["qid"],  # in order to use the API
                "t-start": ww[0],
                "t-end": ww[1],
                "score": ww[2]
            })
    gt_qid2data = defaultdict(list)
    for dd in ground_truth:
        gt_windows = dd["relevant_windows"][:max_gt_windows] \
            if max_gt_windows is not None else dd["relevant_windows"]
        qid = dd["qid"]
        for ww in gt_windows:
...
def compute_mr_r1(submission, ground_truth, iou_thds=np.linspace(0.5, 0.95, 10)):
    ...
    iou_thds = [float(f"{e:.2f}") for e in iou_thds]
    pred_qid2window = {dd["qid"]: dd["pred_relevant_windows"][0][:2] for dd in submission}  # :2 rm scores
    # pred_qid2window = {dd["qid"]: dd["pred_relevant_windows_score_all"][0][:2] for dd in submission}  # :2 rm scores
    
    # gt_qid2window = {d["qid"]: d["relevant_windows"][0] for d in ground_truth}
    gt_qid2window = {}
    for dd in ground_truth:
        cur_gt_windows = dd["relevant_windows"]
        cur_qid = dd["qid"]
        cur_max_iou_idx = 0
        if len(cur_gt_windows) > 0:  # select the GT window that has the highest IoU
            cur_ious = compute_temporal_iou_batch_cross(
                np.array([pred_qid2window[cur_qid]]), np.array(dd["relevant_windows"])
            )[0]
...
def get_data_by_range(submission, ground_truth, len_range):
    ...
    for dd in ground_truth:
        rel_windows_in_range = [
            w for w in dd["relevant_windows"] if min_l < get_window_len(w) <= max_l]
        if len(rel_windows_in_range) > 0:
            dd = copy.deepcopy(dd)
            dd["relevant_windows"] = rel_windows_in_range
            ground_truth_in_range.append(dd)
            gt_qids_in_range.add(dd["qid"])
    # keep only submissions for ground_truth_in_range
    submission_in_range = []
    for dd in submission:
        if dd["qid"] in gt_qids_in_range:
            submission_in_range.append(copy.deepcopy(dd))
...
def eval_submission(submission, ground_truth, verbose=True, match_number=False):
    ...
    # import ipdb; ipdb.set_trace()
    # if "pred_relevant_windows_score_all" in submission[0]:
